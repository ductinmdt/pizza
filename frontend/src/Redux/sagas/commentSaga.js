...
export function* onLoadCommentStartAsync() {
    try {
        ...
        if (res.status === 200) {
            yield delay(500);
            yield put(loadCommentSuccess(res?.data?.elements));
        }
    } catch (error) {
        yield put(loadCommentError(error.res?.data?.elements));
    }
}
...
export function* onCreateCommentStartAsync({ payload }) {
    try {
        ...
        if (res.status === 201) {
            yield put(createCommentSuccess(res?.data?.elements))
        }
    } catch (error) {
        yield put(createCommentError(error.res?.data?.elements));
    }
}
