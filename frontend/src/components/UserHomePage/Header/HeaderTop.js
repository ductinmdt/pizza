...
const HeaderTop = ({ showMenu }) => {
  ...
  const userUI = JSON.parse(localStorage.getItem("userUI")) || JSON.parse(localStorage.getItem("userAdmin")) || null;
  ...
  const newProduct = products.filter((item) => {
    return (
      ...
      item?.category?.categoryName?.toUpperCase().includes(lowercasedFilter.toUpperCase())
    );
  });
                ...
                <div className="account-title dropdown ">
                  <div className="hash-child">
                    ...
                    <ul className="sub-menu">
                      ...
                      <li>
                        <Link to="/userUI/settings">Cài đặt</Link>
                      </li>
                      <li>
                        <span className="text-dark pl-2" onClick={handleLogout}>
                          Đăng xuất
                        </span>
                      </li>
                    </ul>
