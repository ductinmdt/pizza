...
const Coffee = () => {
    ...
    const categoriesList = products.filter(item => item.category_id === Number(categoryId));
    ...
    useEffect(() => {
        const sortArray = (type) => {
            if (type === "priceNewAsc") {
                ...
                const sorted = [...categoriesList].sort((a, b) => a[sortProperty] - b[sortProperty])
                setData(sorted);
                ...
                const sortProperty = types[type];
                const sorted = [...categoriesList].sort((a, b) => b[sortProperty] - a[sortProperty])
                setData(sorted);
            } else
                if (type === "productNameAsc") {
                    ...
                    const sortProperty = types[type];
                    const sorted = [...categoriesList].sort((a, b) => a[sortProperty] === b[sortProperty] ? 0 : a[sortProperty] > b[sortProperty] ? 1 : -1)
                    setData(sorted);
    ...
    const onChange = (value) => {
        setData([...categoriesList].filter(item => (
            item.priceNew <= value[1] && value[0] <= item.priceNew
        )))
    };
    const onAfterChange = (value) => {
        setData([...categoriesList].filter(item => (item.priceNew <= value[1] && value[0] <= item.priceNew)));
        setValue1(value[0])
        setValue2(value[1])
    };
    const marks = {
    ...
    return (
        <>
            <div className="container">
                <BreadCrumb title={products[0]?.category?.categoryName} />
            </div>
            <div className="products mb-5">
                <div className="container">
                    <h4 className="text-uppercase">Các loại {products[0]?.category?.categoryName}</h4>
                    <>
