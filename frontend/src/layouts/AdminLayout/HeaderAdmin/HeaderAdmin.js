...
const HeaderAdmin = ({ handleToggle }) => {
    ...
    const newCommentToday =  comments?.filter(item => moment(item?.createdAt).format("DD/MM/YYYY") === today)
    ...
    const content = (
        <>
            <Link to={`/admin/admins/detail/${profile.id}`}>
    ...
    const header = (
        <div className="profile-header">
            ...
            <span className="profile-role">{profile.role?.name}</span>
        </div>
    )
    const comment = (
        <>
            {newComments && newComments.map((item, index) => {
                return (
                    <div key={index} className="newComments">
                        ...
                        />
                        <span className="username">{item?.user?.fullname}</span>{' '}đã bình luận về <span>{item?.product?.productName}</span>
